...
bool UnZip::contains(const QString& file) const
{
    ...
    for (int i=0; i<d->headers->size(); i++)
        if (d->headers->at(i)->entry_name == file)
            return true;
    return false;
}
...
QList<UnZip::ZipEntry> UnZip::entryList() const
{
    ...
    if (d->headers != 0)
    {
        for (QList<ZipEntryP*>::ConstIterator it = d->headers->constBegin(); it != d->headers->constEnd(); ++it)
        {
            const ZipEntryP* entry = *it;
            Q_ASSERT(entry != 0);

            ZipEntry z;

            z.filename = entry->entry_name;
...
UnZip::ErrorCode UnZip::extractAll(const QDir& dir, ExtractionOptions options)
{
    ...
    bool end = false;
    for (QList<ZipEntryP*>::Iterator itr = d->headers->begin(); itr != d->headers->end(); ++itr)
    {
        ZipEntryP* entry = *itr;
        Q_ASSERT(entry != 0);
        ...
        switch (d->extractFile(entry->entry_name, *entry, dir, options))
        {
        case Corrupted:
        {
            qDebug() << "Removing corrupted entry" << entry->entry_name;
            QList<ZipEntryP*>::Iterator itr1 = itr;
            itr++;
            d->headers->erase(itr1);
...
{
    int i=0;
    for (; i < d->headers->size(); i++)
        if (d->headers->at(i)->entry_name == filename)
            break;
    if (i >= d->headers->size())
        return FileNotFound;

    ZipEntryP* entry = d->headers->at(i);
    Q_ASSERT(entry != 0);
    return d->extractFile(entry->entry_name, *entry, dir, options);
}
...
UnZip::ErrorCode UnZip::extractFile(const QString& filename, QIODevice* dev, ExtractionOptions options)
{
    ...
    ZipEntryP* entry = d->headers->at(i);
    Q_ASSERT(entry != 0);
    return d->extractFile(entry->entry_name, *entry, dev, options);
}
